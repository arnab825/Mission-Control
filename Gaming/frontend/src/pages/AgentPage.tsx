        ...
        <div className="relative group flex items-start justify-end">
          ...
          {role === 'agent' && onToggleTTS && (
            <button
              type="button"
              onClick={onToggleTTS}
              className={`absolute -right-9 top-2 p-1.5 rounded-lg opacity-0 group-hover:opacity-100 transition-all duration-200 border cursor-pointer ${isTTSEnabled
...
const AgentPage: React.FC<{
  ...
  setIsIntelOpen: propSetIsIntelOpen
    ...
    useEffect(() => {
      if (state?.config?.privacy?.enabled !== undefined && localPrivacyEnabled === null) {
        setLocalPrivacyEnabled(state.config.privacy.enabled);
      }
    }, [state?.config?.privacy?.enabled, localPrivacyEnabled]);
    ...
    useEffect(() => {
      if (isVerifyModalOpen && !isGeneratingKeys) {
        ...
        <div className={`flex-1 flex flex-col min-w-0 overflow-hidden relative ${isPopup ? 'bg-[#0a0a10] border border-white/10 rounded-2xl shadow-2xl' : ''}`}>
            ...
            <div className="max-w-2xl mx-auto">
              ...
              {!isCompact && (
                <div role="button" tabIndex={0} onKeyDown={(e) => e.key === 'Enter' && e.currentTarget.click()}
                  ...
                  className="w-full flex justify-center mb-4 select-none cursor-pointer group"
                >
                  {isPrivacyEnabled ? (
                    <div className="flex items-start gap-3 w-full max-w-full sm:max-w-xl px-4 py-3 rounded-xl bg-neon-yellow/5 border border-neon-yellow/15 text-neon-yellow/90 shadow-[0_4px_16px_rgba(0,0,0,0.25)] backdrop-blur-md group-hover:border-neon-yellow/30 group-hover:bg-neon-yellow/10 group-hover:scale-[1.01] active:scale-[0.98] transition-all duration-200 cursor-pointer">
                      <ShieldCheck className="w-4 h-4 text-neon-yellow shrink-0 mt-0.5 animate-pulse" />
                      <div className="text-[11px] leading-relaxed font-semibold min-w-0 flex-1">
                        <span className="font-extrabold text-neon-yellow uppercase tracking-widest mb-0.5 text-[9px] flex items-center gap-1.5 flex-wrap">
                          Privacy Shield Active
                          <span className="text-[8px] text-neon-yellow font-bold normal-case tracking-normal opacity-70">(Click to verify)</span>
                        </span>
                        <div>Messages and telemetry in this stream are end-to-end encrypted. Click anywhere on this card to verify the active cryptographic handshake keys.</div>
                      </div>
                    </div>
                  ) : (
                    <div className="flex items-start gap-3 w-full max-w-full sm:max-w-xl px-4 py-3 rounded-xl bg-amber-500/5 border border-amber-500/15 text-amber-500/90 shadow-[0_4px_16px_rgba(0,0,0,0.25)] backdrop-blur-md group-hover:border-amber-500/30 group-hover:bg-amber-500/10 group-hover:scale-[1.01] active:scale-[0.98] transition-all duration-200 cursor-pointer">
                      <ShieldCheck className="w-4 h-4 text-amber-500 shrink-0 mt-0.5" />
                      <div className="text-[11px] leading-relaxed font-semibold min-w-0 flex-1">
                        <span className="font-extrabold text-amber-500 uppercase tracking-widest mb-0.5 text-[9px] flex items-center gap-1.5 flex-wrap">
                          Privacy Shield Disabled
                          <span className="text-[8px] text-amber-500 font-bold normal-case tracking-normal opacity-70">(Click to verify status)</span>
                        </span>
                        <div>Session logging is currently unencrypted. Click anywhere on this card to verify motherboard UUID status and security settings.</div>
                      </div>
                    </div>
                  )}
                </div>
          ...
          >
            <form onSubmit={handleSubmit} className="max-w-2xl mx-auto">
              <div className={`flex items-center gap-2 sm:gap-3 bg-white/3 border rounded-2xl ${isCompact ? 'p-0.5 pl-2' : 'p-1 pl-3 sm:p-1.5 sm:pl-5'} transition-all ${isDragOver
            ...
            <div className="fixed inset-0 z-[99999] flex items-center justify-center p-4">
              ...
              >
                ...
                <div className="relative text-center mb-4">
                  <div className={`w-10 h-10 rounded-xl mx-auto flex items-center justify-center border mb-2 transition-all duration-500 ${isGeneratingKeys
                ...
                <div className={`relative w-28 h-28 mx-auto mb-4 bg-black/40 border rounded-xl flex items-center justify-center overflow-hidden transition-all duration-500 ${isPrivacyEnabled ? 'border-neon-yellow/15' : 'border-red-500/20'
                  }`}>
                  {/* Dots Matrix */}
                  <div className="grid grid-cols-12 gap-1 opacity-25">
                    {Array.from({ length: 144 }).map((_, i) => {
                      ...
                      return (
                        <div
                          key={i}
                          className={`w-1 h-1 rounded-sm transition-all duration-300 ${isGeneratingKeys
                  ...
                  {(isGeneratingKeys || isPrivacyEnabled) && (
                    <motion.div
                      ...
                      className={`absolute left-0 right-0 h-0.5 pointer-events-none opacity-60 shadow-[0_0_10px_currentColor] ${isGeneratingKeys ? 'bg-neon-green text-neon-green' : 'bg-neon-yellow text-neon-yellow'
                        }`}
                    />
                  )}
                ...
                <div className="space-y-2 mb-4">
                  <div className="text-center flex items-center justify-center gap-2">
                    ...
                    {!isGeneratingKeys && (
                      <span className={`text-[7px] font-bold uppercase tracking-wider px-1.5 py-0.5 rounded-full border ${isPrivacyEnabled
                  ...
                  <div className="grid grid-cols-4 gap-1.5">
                    {secKeys.map((key, i) => (
                      <div
                        key={i}
                        className={`py-1.5 px-0.5 text-center rounded-lg font-mono text-[9px] font-bold border transition-all duration-300 ${isGeneratingKeys
                ...
                <button aria-label="button" type="button"
                  ...
                  className={`w-full py-2 px-3 rounded-xl border font-bold text-[10px] uppercase tracking-widest transition-all active:scale-95 flex items-center justify-center gap-2 cursor-pointer ${isPrivacyEnabled
