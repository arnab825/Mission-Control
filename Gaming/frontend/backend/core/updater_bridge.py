...
def handle_bridge_update_commands(cmd_type: str, payload: dict, bridge_instance) -> bool:
    ...
    if cmd_type == "get_changelogs":
        try:
            ...
            changelog = local_copy.get("changelog", [])
            now_str = datetime.now().strftime("%Y-%m-%d")
            local_copy["changelog"] = [log for log in changelog if log.get("date", "") <= now_str]
        ...
        def _check_run():
            try:
                ...
                if Version(remote_ver) > Version(local_ver):
                    ...
                    now_str = datetime.now().strftime("%Y-%m-%d")
                    remote_log = [log for log in remote_log if log.get("date", "") <= now_str]
                    local_log = [log for log in local_log if log.get("date", "") <= now_str]
                ...
                bridge_instance.update_state(
                    {"update_state": {"status": "failed", "reason": str(e)}}
                )
                    ...
                    bridge_instance.update_state(
                        {
                            "update_install_state": {
                                "status": "failed",
                                "reason": f"uv sync failed: {result.stderr.strip()[:100]}",
                            }
                        }
                    )
                    return
