...
def _try_win_cpu_temp_native() -> float:
    ...
    0. AWCC WMI (Alienware Command Center) - works on all Alienware laptops, no driver needed
    1. PDH 'Thermal Zone Information' performance counters (works on most modern Intel laptops)
    2. PowerShell Get-Counter thermal zone (broader compatibility)
    ...
    def run(self):
        ...
        def start_lhm_helper():
            ...
            if not os.path.exists(dll_path):
                dll_path = os.path.join(base_dir, "system", "hardware_monitor", "bin", "Debug", "net10.0", "HardwareMonitor.dll")
            
            if not os.path.exists(dll_path):
                logger.warning(f"[Telemetry] LibreHardwareMonitor helper DLL not found at {dll_path}.")
                return None
        ...
        while self.running and (self.p.running or getattr(self.p, "headless", False)):
            try:
                ...
                if lhm and "cpu_pct" in lhm and float(lhm["cpu_pct"]) > 0:
                    cpu_pct = float(lhm["cpu_pct"])
                ...
                if not getattr(self, "_cpu_max_freq_resolved", False):
                    ...
                    if cpu_name and cpu_name != "Detecting..." and cpu_name != "Unknown CPU":
                        ...
                        # Temporary fallback until specs are discovered
                        if self._cached_cpu_max_freq <= 0:
                            ...
                            try:
                                ...
                                pass
                            self._cached_cpu_max_freq = max_f if max_f > 0 else 2400
                            self._cpu_base_freq = max_f if max_f > 0 else 2400
                ...
                # Fallback to psutil if PDH is not available or returned 0
                if cpu_freq <= 0:
                    try:
                        cf = psutil.cpu_freq() if hasattr(psutil, "cpu_freq") else None
                ...
                if platform.system() == "Windows":
                            ...
                            try:
                                ...
                                try:
                                    ...
                                    if len(out) >= 1 and out[0].strip().isdigit():
                                        t = int(out[0].strip())
                                        if t > 0:
                                            self._cached_wmi_temp = (t - 2732) / 10.0
                                    ...
                                    try:
                                        ...
                                        if "CurrentTemperature=" in out:
                                            val = out.split("CurrentTemperature=")[-1].strip()
                                            if val.isdigit():
                                                t = int(val)
                                                if t > 0:
                                                    self._cached_wmi_temp = (t - 2732) / 10.0
                    ...
                    # Always poll AWCC every 2s (independent of whether we have a reading)
                    # so the temperature stays live and dynamic, not frozen at first reading.
                    if not hasattr(self, "_last_native_temp_poll") or (now - self._last_native_temp_poll) > 2.0:
                        ...
                        if t > 0:
                            ...
                            logger.debug(f"[Telemetry] AWCC thermal read: {t}°C")
