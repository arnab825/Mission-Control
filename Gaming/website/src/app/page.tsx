...
export default function Home() {
  ...
  return (
    <div className="min-h-screen flex flex-col items-center justify-start w-full relative overflow-hidden pt-20 sm:pt-24 bg-obsidian text-white">
      ...
      {/* ================= HERO SECTION (MODERN 2-COLUMN WINDOWS LAYOUT) ================= */}
          ...
          {TESTED_GAMES_LIST.map((game) => (
            ...
            >
              ...
              <div className="p-5 sm:p-6 flex flex-col justify-between flex-1 space-y-4 sm:space-y-5">
                ...
                {/* View Profile Button (Premium Tactile Class) */}
                <Link
                ...
                {["Tactical", "Immersive", "Friendly", "Sarcastic", "Aggressive"].map((p) => {
                  ...
                  return (
                    <button
                      ...
                      className={`py-2.5 px-3 sm:px-2 md:px-3 rounded-lg text-[11px] font-mono font-bold transition-all cursor-pointer whitespace-nowrap sm:whitespace-normal shrink-0 sm:shrink ${isActive
              ...
              <div className="grid grid-cols-3 gap-1 sm:gap-2 p-1.5 bg-obsidian/90 border border-white/10 rounded-2xl font-mono text-[10px] sm:text-xs w-full shadow-inner">
                {(["horizontal", "compact", "standard"] as const).map((tab) => (
                  <button
                    ...
                    className={`w-full py-2.5 px-1 sm:px-3 rounded-xl font-bold uppercase transition-all cursor-pointer truncate text-center ${activeHudTab === tab
        ...
        <div className="space-y-4">
          {faqs.map((faq, idx) => {
            ...
            return (
              <div
                key={idx}
                className={`glass-card transition-all duration-300 overflow-hidden ${isOpen ? "border-gradient-cyber shadow-[0_0_25px_rgba(118,185,0,0.2)]" : "hover:border-white/25"
                  }`}
              >
                <button
                ...
                >
                  <span className={`font-bold text-sm sm:text-lg transition-colors leading-snug ${isOpen ? "text-neon-green glow-text-teal" : "text-white"}`}>
                    {faq.q}
                  </span>
                  <div className={`w-8 h-8 rounded-full flex items-center justify-center shrink-0 transition-all duration-300 ${isOpen ? "rotate-180 icon-badge-premium shadow-[0_0_15px_rgba(118,185,0,0.5)] text-neon-green" : "bg-white/5 border border-white/10 text-gray-400"
                    }`}>
